...
public class FFIBindingTable {
    ...
    public List<String> getAllIncludes() {
        return Collections.unmodifiableList(globalIncludes);
    }

    public Map<String, ExternType> getAllTypes() {
        return Collections.unmodifiableMap(globalTypes);
    }

    public Map<String, ExternConstant> getAllConstants() {
        return Collections.unmodifiableMap(globalConstants);
    }
    ...
    public List<LinkDirective> getAllLinks() {
        return Collections.unmodifiableList(globalLinks);
    }
    ...
    public static class ExternBlock {
        ...
        // ===== 块级元数据 =====
        public PlatformConstraint platform = null;  // NEW: 平台约束
    ...
    /**
     * 平台约束 — 挂载在 ExternBlock 上
     *
     * 支持三个维度的过滤： 1. 操作系统 (platform) 2. CPU 架构 (architecture) 3. 编译器/工具链
     * (toolchain) — 可选
     */
    public static class PlatformConstraint {

        // ====== 操作系统 ======
        private final Set<String> platforms = new LinkedHashSet<>(); // 允许的平台: "windows", "linux", "macos", "android", "ios"

        // ====== CPU 架构 ======
        private final Set<String> architectures = new LinkedHashSet<>();  // 允许的架构: "x86_64", "arm64", "x86", "arm"

        // ====== 工具链 (可选) ======
        private final Set<String> toolchains = new LinkedHashSet<>();
        ...
        public static final String WASM32 = "wasm32";
        ...
        public static final PlatformConstraint UNIVERSAL = null;
        ...
        public PlatformConstraint addArchitecture(String... archs) {
            Collections.addAll(this.architectures, archs);
            return this;
        }

        public PlatformConstraint addToolchain(String... toolchains) {
            Collections.addAll(this.toolchains, toolchains);
            return this;
        }
        ...
        public boolean matches(TargetTriple target) {
            if (platforms.isEmpty() && architectures.isEmpty() && toolchains.isEmpty()) {
                return true; // UNIVERSAL constraint
            }

            boolean platformOK = platforms.isEmpty() || platforms.contains(target.platform);
            boolean archOK = architectures.isEmpty() || architectures.contains(target.architecture);
            boolean toolchainOK = toolchains.isEmpty() || toolchains.contains(target.toolchain);

            return platformOK && archOK && toolchainOK;
        }

        public boolean isUniversal() {
            return platforms.isEmpty() && architectures.isEmpty() && toolchains.isEmpty();
        }

        public List<String> getPlatforms() {
            return platforms.isEmpty() ? Collections.emptyList() : new ArrayList<>(platforms);
        }
    ...
    public FFIBindingTable filterForPlatform(TargetTriple target) {
        ...
        for (ExternBlock block : getExternBlocks()) {
            if (block.platform != null && !block.platform.matches(target)) {
                continue;
            }
            ...
            for (ExternFunction func : block.functions) {
                if (func.platformConstraint == null || func.platformConstraint.matches(target)) {
