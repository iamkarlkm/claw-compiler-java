...
public class PythonFFIGenerator {
    ...
    public String generateFunctionBindings() {
        ...
        // 需要确定每个函数属于哪个库
        // 简化策略：使用第一个 link 的库（如果只有一个库）
        // 完整策略：需要在 extern 块中关联函数与库
        String defaultLibVar = getDefaultLibraryVar();

        StringBuilder sb = new StringBuilder();
        sb.append("# ========== External Function Signatures ==========\n");

        for (ExternFunction func : functions.values()) {
            String libVar = defaultLibVar;
    ...
    public String generateWrapperFunctions() {
        ...
        for (ExternFunction func : functions.values()) {
            sb.append(generateSingleWrapper(func, defaultLibVar));
            sb.append("\n");
        }
