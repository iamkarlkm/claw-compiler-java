...
public class EnhancedCompletionProvider extends CompletionProvider {
        ...
        try {
            ...
            switch (context) {
                ...
                case ANNOTATION:
                    ...
                    break;
                case KEYWORD:
                    completions.addAll(getKeywordCompletions());
                    break;
                case VARIABLE:
            ...
            completions.sort((a, b) -> {
                // 优先显示最近使用的项
                int aPriority = a.getRelevance() != null ? a.getRelevance() : 0;
                int bPriority = b.getRelevance() != null ? b.getRelevance() : 0;
            ...
            performanceMonitor.end(OPERATION_NAME, startTime);
            return convertToLsp4j(completions);
    ...
    private Context analyzeContextEnhanced(String document, Position position) {
        ...
        // 检查是否是修饰符
        if (isModifierStart(linePrefix)) {
            return Context.MODIFIER;
        }

        // 检查是否是变量
        if (isVariableStart(linePrefix)) {
