...
public class JavaCodeGenerator implements TargetCodeGenerator {
    ...
    private void generateInstruction(IRInstruction inst) {
        ...
        switch (inst.getOpCode()) {
            ...
            // ==================== AOP (Aspect-Oriented Programming) 操作码 ====================
            case BEFORE_ADVICE: {
                // @Before 通知
                String adviceName = ops.get(0).toString();
                String targetName = ops.size() > 1 ? ops.get(1).toString() : "<target>";
                appendLine(runtime.generateBeforeAdvice(adviceName, targetName));
                break;
            }
            ...
            case AFTER_ADVICE: {
                ...
                String adviceName = ops.get(0).toString();
                String targetName = ops.size() > 1 ? ops.get(1).toString() : "<target>";
                appendLine(runtime.generateAfterAdvice(adviceName, targetName));
                break;
            }

            case AFTER_RETURNING_ADVICE: {
                // @AfterReturning 通知
                String adviceName = ops.get(0).toString();
                String targetName = ops.size() > 1 ? ops.get(1).toString() : "<target>";
                String returnVar = ops.size() > 2 ? ops.get(2).toString() : "returnValue";
                appendLine(runtime.generateAfterReturningAdvice(adviceName, targetName, returnVar));
                break;
            }

            case AFTER_THROWING_ADVICE: {
                // @AfterThrowing 通知
                String adviceName = ops.get(0).toString();
                String targetName = ops.size() > 1 ? ops.get(1).toString() : "<target>";
                String exceptionVar = ops.size() > 2 ? ops.get(2).toString() : "ex";
                appendLine(runtime.generateAfterThrowingAdvice(adviceName, targetName, exceptionVar));
                break;
            }
            ...
            case AROUND_ADVICE: {
                ...
                String adviceName = ops.get(0).toString();
                String targetName = ops.size() > 1 ? ops.get(1).toString() : "<target>";
                appendLine(runtime.generateAroundAdvice(adviceName, targetName));
                break;
            }
            ...
            case JOIN_POINT_CREATE: {
                ...
                String methodName = ops.size() > 1 ? ops.get(1).toString() : "unknown";
            ...
            case METHOD_INVOCATION: {
                ...
                List<String> args = ops.subList(1, ops.size()).stream()
                    .map(Object::toString)
                    .collect(Collectors.toList());
                appendLine(runtime.generateMethodInvocation(methodName, args));
                break;
            }

            case ADVICE_PROCEED: {
                // Proceed 调用（Around 通知中）
                String joinPointName = ops.get(0).toString();
                appendLine(runtime.generateAdviceProceed(joinPointName));
                break;
            }
    ...
    public GenerationResult generate(ClawIR ir, GenerationConfig config) {
        ...
        try {
            ...
            // 5. 遍历 IR 块生成代码
            for (IRGenerator.IRBasicBlock block : ir.getNodes()) {
                generateBlock(block);
            }
